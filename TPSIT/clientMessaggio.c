...
int main(){
  ...
  puts("CLIENT: Ho inviato il messaggio al server.\n");

	printf("CLIENT: Risposta del server:\n\t");
	ricevi(sock, buffer);	// Salvo nel buffer la risposta ricevuta
	puts(buffer);			// Stampo il buffer. Se c'� stato un errore, nel buffer c'� salvato un messaggio di errore.
}
