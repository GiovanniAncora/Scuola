 ...
 * Date di scrittura del programma: 03/12/2024, 07/12/2024
...
int main(){
	...
	int conferma = 1;
	server = creaId_dispositivo(8080, "0.0.0.0", &conferma);	// Mettere "0.0.0.0" è come usare INADDR_ANY. Il server accetterà richieste da qualsiasi sua interfaccia.
	if(conferma != 0){
	...
	// A questo punto la connessione con il client è avvenuta, quindi provo a vedere che mi chiede
	if(ricevi(nuovoSocket, buffer) == -1){
		perror("SERVER: Errore nella ricezione della request");
		return 0;
	}

	printf("SERVER: Request ricevuta dal client:\n\t%s\n", buffer);

	invia(nuovoSocket, "Ciao, sono il server! Ho ricevuto il tuo messaggio!\n");
