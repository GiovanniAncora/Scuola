...
int main(){
	...
	int conferma;		// Questa variabile serve per salvare e controllare il valore modificato in memoria con un puntatore da creaId_dispositivo, per sapere se l'operazione ï¿½ andata a buon fine
	...
	if(conferma != 0){	// Controllo se l'operazione ï¿½ andata a buon fine
	...
	if (connetti(sock, server) == -1){	// Provo a connettermi con la funzione connetti. Se c'ï¿½ stato un errore, viene restituito -1 e termino.
		return 0;
	}
	...
	while(input >= 1 || input <= 8){
		strcpy(richiesta, convertiLinux(input));
...
struct sockaddr_in creaId_dispositivo(int port, char* ip, int* conferma){
	...
	// La variabile puntatore chiamata conferma serve perchï¿½ lï¿½ scriveremo se l'operazione ï¿½ andata a buon fine o meno, attraverso un valore di flag.
	
	struct sockaddr_in disp;	// Dichiaro la struct che verrï¿½ restituita
	...
	if(inet_aton(ip, &disp.sin_addr)){	// Con inet_aton iene salvato in disp.sin_addr l'ip del dispositivo. Il valore restituito dalla funzione ci permette di controllare se l'ip ï¿½ stato trovato.
		*conferma = 0;	// Caso in cui si ï¿½ trovato l'ip e l'operazione ï¿½ andata a buon fine
		return disp;
	}else{
		*conferma = 1;	// Caso in cui NON si ï¿½ trovato l'ip e l'operazione NON ï¿½ andata a buon fine
		return disp;	// In entrambi i casi restituisco la struct. Ma in questo caso, dato che c'ï¿½ stato un errore, essa non assume valori significativi. Sta a chi ha chiamato la funzione gestire l'errore.
	}
}
...
int connetti(int sock, struct sockaddr_in server){
	...
	// L'intero restituito ï¿½ -1 se c'ï¿½ stato un errore.
...
int ricevi(int sock, char* buffer){
	// La funzione ricevi permette di salvare nel buffer passato come parametro ciï¿½ che l'altro dispositivo ha inviato (in questo caso il server al client).
	// Viene restituito -1 se c'ï¿½ stato un errore, 0 se ï¿½ tutto OK.
	
	int start = clock();	// Per fare il controllo di timeout, salvo in start il numero di clock prima di attendere la risposta
	int now = clock();		// Anche in questa salvo il numero di clock attuali. Questa variabile perï¿½ si aggiornerï¿½ per poter salvare il tempo.

    while((now - start) < TIMEOUT){	// Continuo a fare l'operazione di ricevere finchï¿½ non passa un intervallo di tempo definito nella costante TIMEOUT
        if((recv(sock, buffer, 4096, 0)) > 0){	// Quando la funzinoe recv restituirï¿½ un valore maggiore di 0, avrï¿½ ricevuto la risposta, quindi posso salvarla nel buffer e chiudere la connessione.
          shutdown(sock, 2);
          return 0;
        }
	...
	return -1;	// Restituisco -1 cosï¿½ faccio sapere al programma main che c'ï¿½ stato un errore.
}
